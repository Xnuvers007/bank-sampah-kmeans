...
// Ambil ID nasabah dari session (dibuat saat login_process.php)
...
try {
    ...
    $stmt_riwayat = $pdo->prepare("
        ...
        FROM transaksi_tarik WHERE id_nasabah = ?)
...
<head>
    ...
    <style>
        /* Desain sederhana untuk nasabah */
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body text-center">
                        <?php if ($profil['id_klaster'] !== null): ?>
                            <span class_badge="badge bg-success cluster-badge">
                                <i class="fas fa-star"></i> <?= htmlspecialchars($profil['nama_klaster']) ?>
                            </span>
                            <p class="mt-3 text-muted"><?= htmlspecialchars($profil['deskripsi']) ?></p>
                        <?php else: ?>
                            <span class="badge bg-secondary cluster-badge">
                                <i class="fas fa-question-circle"></i> Belum Terklasifikasi
                            </span>
                    ...
                    <div class="card-body">
                        ...
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                ...
                                <span class="badge bg-primary rounded-pill"><?= number_format($agregat['total_berat'], 2) ?> kg</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                Frekuensi Setoran
                                <span class="badge bg-primary rounded-pill"><?= $agregat['frekuensi_setor'] ?> kali</span>
                            </li>
                        </ul>
                    </div>
                </div>
                        ...
                        <ul class="list-group list-group-flush">
                            <?php foreach ($riwayat as $r): ?>
                                <li class="list-group-item">
                                    <div class="d-flex w-100 justify-content-between">
                                        <h6 class="mb-1">
                                            <?php if ($r['jenis'] == 'Setor Sampah'): ?>
                                                <i class="fas fa-arrow-down text-success"></i> <?= $r['jenis'] ?>
                                            <?php else: ?>
                                                <i class="fas fa-arrow-up text-danger"></i> <?= $r['jenis'] ?>
                                            <?php endif; ?>
                                        </h6>
                                    ...
                                    <p class="mb-1">
                                        <?php if ($r['jenis'] == 'Setor Sampah'): ?>
                                            Mendapat <span class="text-success">+<?= format_rupiah($r['jumlah']) ?></span>
                                        <?php else: ?>
                                            Menarik <span class="text-danger">-<?= format_rupiah($r['jumlah']) ?></span>
                                        <?php endif; ?>
                                    </p>
                                </li>
                            <?php endforeach; ?>
                            <?php if (empty($riwayat)): ?>
                                <li class="list-group-item text-center text-muted">Belum ada riwayat transaksi.</li>
                            <?php endif; ?>
                        </ul>
